 ...
 * @return array|int
 */
function smarty_function_query_kvs_custom($params, &$smarty)
{
	...
	foreach ($params as $k => $v)
	{
		...
		{
			$wherelike[substr($k, 10)] = mysql_real_escape_string($v);
		}
		if (strpos($k, 'wherelikereg_') === 0)
		{
			$wherelikereg[substr($k, 13)] = mysql_real_escape_string($v);
		}
	}
